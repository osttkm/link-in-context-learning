...
def prepare_data(
    ...
    #     print(f'use_icl:{use_icl}')
    #     print(f'shot num:{shot}')
    #     print(f'use_mix:{mix}')
    #     print(f'icl_dataset_list:{icl_dataset_list}')
    ...
    if hasattr(data_args, 'multitest') and bool(data_args.multitest) \
        ...
        for k, item in data_args.multitest.items():
            ...
            if training_args.local_rank == 0:
                print(f'item["compute_metric"]: {item["compute_metric"]}')
    ...
    if training_args.local_rank == 0:
        print(f'compute_metric: {compute_metrics}')
...
def build_compute_metric(compute_metric_cfg, preprocessor):
    if compute_metric_cfg is not None:
        ...
        # if training_args.local_rank == 0:
        #     print('-----------------conpute_metric_cfg-----------------')
        #     print(compute_metric_cfg)
