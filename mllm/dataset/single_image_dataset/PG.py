...
class CustomDatasetTrain_PG(LCLDataset):
    def __init__(self, policy: str, *args, **kwargs):
        super().__init__(*args, **kwargs)
        print('==========TASK PG is initialized==========')
        ...
        custom_json = None
        # jsonファイルの読み込み
        with open('/home/oshita/vlm/Link-Context-Learning/config/_base_/dataset/template/PG.json', 'r') as f:
    ...
    def policy_2way_weight(self, index, shot):
        ...
        def custom_convert_answer(mode, idx):
            assert mode in ['cls_negative', 'neighbors']
            answer = self.pg_answer[idx][0].format(product=self.product_name)
        ...
        idx = random.randint(0, len(self.pg_context_question)-1)
        infer_question = self.pg_context_question[idx][0]
