...
def init_Qformer(num_query_token, vision_width, pretrained_path,ckpt_path,cross_attention_freq=2,):
    ...
    if rank == 0:
        print(f'Qformer pretrained path : {pretrained_path}')
        print(f'Qformer config : {model_dict}')
...
class LlavaLlamaModel(LlamaModel):
    ...
    def __init__(self, config: LlamaConfig, mm_vision_tower=None, mm_hidden_size=None):
        ...
        self.swin = Swinv2Model(swinv2_config)
            ...
            if len(images.shape) == 5:
                ...
                for image in images_list:
                    ...
                    if self.qformer_config is not None:
                        ...
                        print(f'query_tokens: {self.query_tokens}')
                ...
                for cur_idx, (cur_input_ids, cur_input_embeds) in enumerate(zip(input_ids, inputs_embeds)):
                    ...
                    if (cur_input_ids == vision_tower.config.im_patch_token).sum() == 0:
                        ...
                        continue
                    # image tokenの使用が許されている場合（複数画像が入力の時？？）
...
class LlavaLlamaForCausalLM(LlamaForCausalLM):
    ...
    def __init__(self, config):
        print('========LlavaLlamaForCausalLM is initialized========')
        super(LlamaForCausalLM, self).__init__(config)
        # ここでのモデルはQformnerに関しての設定がなければ画像特徴のみ抽出してCAを取らないやつ
        self.model = LlavaLlamaModel(config)
        self.lm_head = nn.Linear(config.hidden_size, config.vocab_size, bias=False)
        if hasattr(config, "use_weighted_tokens_path"):
            print(config)
            print(f'==================use_weighted_tokens_path is True==================')
            self.weights_path = config.use_weighted_tokens_path
            self.weights = torch.tensor(np.load(self.weights_path))
        else:
            print(config)
            print(f'==================use_weighted_tokens_path is False==================')
            self.weights = None
    ...
    def get_yesno_loss(self):
        return self.yesno_loss
    ...
    def forward(
        ...
        if labels is not None:
            ...
            if self.weights == None:
                ...
                weights = torch.ones(self.config.vocab_size).to(shift_logits.device)
                for w in self.weights:
                    weights[w] = 2
                loss_fct = CrossEntropyLoss(weight=weights.to(shift_logits.device))
            ...
            self.yesno_loss = loss_fct(shift_logits[shift_labels==3869], shift_labels[shift_labels==3869])+loss_fct(shift_logits[shift_labels==1939], shift_labels[shift_labels==1939])
            self.yesno_loss = self.yesno_loss/2
    ...
    def initialize_vision_tokenizer(self, mm_use_im_start_end, tokenizer, device,
        ...
        if mm_use_im_start_end:
            ...
            # tokenizerの長さを再設定
            self.resize_token_embeddings(len(tokenizer))
            # imageトークンに関してvision_configにも変更を反映
            vision_config.im_start_token, vision_config.im_end_token = tokenizer.convert_tokens_to_ids([DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN])

            # 追加トークンがあった際の変更＝imageトークンが追加されたとき
            # LLMの埋め込みを学習する
            if num_new_tokens > 0:
            ...
            # 言語埋め込みを学習可能にするパラメータ，新しい特殊トークンとかを辞書に追加，その後埋め込むときには追加学習が必要になる
            # 基本的には不要かもしれないが，自作も出るとか作る際は必要なのでしっかりと理解しておくこと
            if tune_mm_mlp_adapter:
                print('##############  Language adapter is trainable  ###############')
...
AutoConfig.register("oshita_llava", LlavaConfig)
