...
#         super().__init__(*args, **kwargs)
#         self.all_probs = []  # 新しい属性を追加して全てのバッチの確率を保持
...
#         print('TrainerForMMLLM prediction_step')
...
#         # Add for getting logits
#         self._gen_kwargs['output_scores'] = True
#         self._gen_kwargs['return_dict_in_generate'] = True
...
#                 generated_outputs = self.model.generate(**gen_kwargs)
        
#         # Extract generated tokens and scores
#         generated_tokens = generated_outputs.sequences
#         generated_scores = generated_outputs.scores
...
#             probs = self._get_token_probabilities(generated_tokens, generated_scores)
#         else:
#             probs = None
        
#         self.all_probs.append(probs)  # 全てのバッチの確率を保存
#         return loss, generated_tokens, labels

#     def _get_token_probabilities(self, generated_tokens, generated_scores):
#         # Convert scores to probabilities for each token
#         batch_size, seq_len = generated_tokens.size()
#         vocab_size = generated_scores[0].size(-1)

#         # Initialize a tensor to hold the probabilities
#         token_probs = torch.zeros(batch_size, seq_len, vocab_size, device=generated_tokens.device)

#         for i in range(seq_len):
#             if i < len(generated_scores):  # Ensure we do not go out of bounds
#                 score = generated_scores[i]  # (batch_size, vocab_size)
#                 prob = torch.softmax(score, dim=-1)  # Convert logits to probabilities
#                 token_probs[:, i, :] = prob
#             else:
#                 # If generated_scores length is less than seq_len, fill with zeros or handle as needed
#                 token_probs[:, i, :] = torch.zeros(batch_size, vocab_size, device=generated_tokens.device)

#         return token_probs

#     # def predict(self, test_dataset: Dataset, ignore_keys: Optional[List[str]] = None, metric_key_prefix: str = "test", **gen_kwargs):
#     #     # Reset the list to store all probabilities
#     #     self.all_probs = []
        
#     #     # Call the original predict method
#     #     prediction_output = super().predict(test_dataset, ignore_keys, metric_key_prefix, **gen_kwargs)
        
#     #     # Find the maximum length
#     #     max_length = max([prob.shape[0] for prob in self.all_probs]) if self.all_probs else 0

#     #     # Pad each probability tensor in self.all_probs to the maximum length
#     #     for i in range(len(self.all_probs)):
#     #         pad_size = max_length - self.all_probs[i].shape[0]
#     #         if pad_size > 0:
#     #             self.all_probs[i] = torch.nn.functional.pad(self.all_probs[i], (0, pad_size), value=-100)

#     #     # Concatenate all probabilitiesd
#     #     all_probs = torch.cat(self.all_probs, dim=0) if self.all_probs else None
        
#     #     return prediction_output, all_probs
...
#             pad_size = max_length - self.all_probs[i].shape[0]
#             if pad_size > 0:
#                 # Provide padding size for both before and after in each dimension
#                 self.all_probs[i] = torch.nn.functional.pad(self.all_probs[i], (0, 0, 0, pad_size), value=-100)
...
class TrainerForMMLLM(TrainerDifferentCollatorMixin, Seq2SeqTrainer):
    ...
    #     # Find the maximum sequence length and maximum batch size
    #     max_seq_length = max([prob.shape[1] for prob in self.all_probs]) if self.all_probs else 0
    #     max_batch_size = max([prob.shape[0] for prob in self.all_probs]) if self.all_probs else 0
        
    #     # Pad each probability tensor in self.all_probs to the maximum sequence length and batch size
    #     for i in range(len(self.all_probs)):
    #         pad_seq_size = max_seq_length - self.all_probs[i].shape[1]
    #         pad_batch_size = max_batch_size - self.all_probs[i].shape[0]
    #         if pad_seq_size > 0 or pad_batch_size > 0:
    #             # Provide padding size for both before and after in each dimension
    #             self.all_probs[i] = torch.nn.functional.pad(self.all_probs[i], (0, 0, 0, pad_seq_size, 0, pad_batch_size))
    ...
    def save_prediction(self, predict_results, file_key_prefix='predict', probs=None):
        ...
        if probs != None:
            # convert probs tensor to numpy array
            probs = probs.cpu().numpy()
            np.save(os.path.join(self.args.output_dir, f"{file_key_prefix}_logits.npy"), probs)
        ...
        # preds, targets, probs = predict_results.predictions, predict_results.label_ids, probs
        preds, targets = predict_results.predictions, predict_results.label_ids
        origin_preds, origin_targets = preds, targets
        # preds, targets, probs = deepcopy(preds), deepcopy(targets), deepcopy(probs)
        ...
        with open(os.path.join(self.args.output_dir, f'{file_key_prefix}_extra_prediction.jsonl'), 'a', encoding="utf-8") as g:
            # for p, t, l, pi, ti in tqdm(
            for p, t, pi, ti in tqdm(
                    # zip(preds, targets, probs, origin_preds, origin_targets),
...
class Seq2Seq2DataCollatorWithImage(Seq2SeqDataCollator):
    ...
    def _image_process(self, features: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for feature in features:
            if len(feature['image'].shape)==3:
                img = feature['image'].unsqueeze(0)
                images.append(img)
            else:
