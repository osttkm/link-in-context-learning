...
# キーが辞書型で、値がConversation型．Cみたいに予約してあるだけ
...
if __name__ == "__main__":
    ...
    print('\n')
    icl_conv = get_conv_template("icl_v1.0")
    icl_conv.append_message(icl_conv.roles[0], "Hello!")
    icl_conv.append_message(icl_conv.roles[1], "Hi!")
    icl_conv.append_message(icl_conv.roles[0], "How are you?")
    icl_conv.append_message(icl_conv.roles[1], None)
    print(icl_conv.get_prompt())
    print('\n')
    my_conv = get_conv_template("Image_only_mode")
    my_conv.append_message(my_conv.roles[0], "<image>")
    my_conv.append_message(my_conv.roles[1], None)
    print(my_conv.get_prompt())
