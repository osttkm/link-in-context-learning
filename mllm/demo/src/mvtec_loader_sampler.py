...
class MVTecAD(Dataset):
    ...
    def __init__(self, root='/home/dataset/mvtec', category='bottle', transform=None, mode='train'):
        ...
        ano_labels = [1 for _ in range(len(ano_img_paths))]

        normal_img_paths = [str(p) for p in Path(self.data_dir+'/good').iterdir() if p.is_file()]
        normal_labels = [0 for _ in range(len(normal_img_paths))]
        ...
        if len(ano_labels) > len(normal_labels):
            ...
            normal_img_paths.extend(add_normal_path)
            normal_labels.extend([0 for _ in range(len(add_normal_path))])
    ...
    def run(self,transform,mode):
        if mode=='train':
            ...
            self.train_loader = DataLoader(
                ...
                pin_memory=True,
                num_workers=4,
                batch_sampler = BalancedBatchSampler(self.train_dataset,2,8),)
...
if __name__ == '__main__':
    ...
    print('setup loader')
    loader = mvtecAD_loader(category='bottle')
    train_dataset = loader.run(transform=transform_img,mode='train')
    for idx,(data,label,_) in enumerate(train_dataset):
        print(data.shape)